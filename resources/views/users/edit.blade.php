                ...
                <div class="px-6 py-4 space-y-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="flex-shrink-0 h-16 w-16 rounded-full overflow-hidden bg-gray-200">
                            <img id="profile-photo-preview" class="h-full w-full object-cover" src="{{ $user->profile_photo ? asset('storage/' . $user->profile_photo) : asset('images/default-user.png') }}" alt="Profile photo preview">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="password"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Leave blank to keep current password">
                        @error('password')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="radio_id" id="radio_id"
                            ...
                            @foreach($radios as $radio)
                                <option value="{{ $radio->id }}" {{ old('radio_id', $user->radio_id) == $radio->id ? 'selected' : '' }}>
                                    {{ $radio->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="role_id" id="role_id" required
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" data-radio="{{ $role->radio_id }}" {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="teams[]" id="teams" multiple
                            ...
                            @foreach($teams as $team)
                                <option value="{{ $team->id }}" data-radio="{{ $team->radio_id }}" 
                                    {{ in_array($team->id, old('teams', $user->teams->pluck('id')->toArray())) ? 'selected' : '' }} 
                                    style="display: none;">
                    ...
                    <div>
                        ...
                        <textarea name="bio" id="bio" rows="3"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('bio', $user->bio) }}</textarea>
                        @error('bio')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="mt-2 space-y-2">
                            ...
                            <div class="flex items-center">
                                <input id="status-desactive" name="status" type="radio" value="desactive"
                                       {{ old('status', $user->status) === 'desactive' ? 'checked' : '' }}
                                       class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        filterOptions(); // Initialize on page load
    });
</script>
@endsection
